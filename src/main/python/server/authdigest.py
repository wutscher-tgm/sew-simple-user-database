...
class RealmDigestDB(object):
    ...
    def __init__(self, realm, db, algorithm='sha256'):
        self.realm = realm
        self.alg = self.newAlgorithm(algorithm)
        self.db = db#self.newDB()
    ...
    def isAuthenticated(self, request, **kw):
        ...
        user = self.db.getUser(authorization.username)#self[authorization.username]
        print('user: '+str(user))
        if user is None:
            return authResult.deny('unknown_user')
        elif not self.alg.verify(authorization, user['password'], request.method, **kw):
    ...
    def challenge(self, response=None, status=401):
        ...
        authReq.set_digest(self.realm, os.urandom(8).hex())
        return response
...
class DigestAuthentication(object):
    ...
    def addDigestHashAlg(klass, key, hashObj):
        ...
        def H(*args):
            x = ':'.join(map(str, args))
            return hashObj(x.encode('utf-8')).hexdigest()
